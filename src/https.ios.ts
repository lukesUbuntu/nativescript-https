...
function AFSuccess(resolve, task: NSURLSessionDataTask, data: NSDictionary<string, any> & NSData & NSArray<any>) {
    ...
    if (data && data.class) {
        ...
        if (data.enumerateKeysAndObjectsUsingBlock || data.class().name === 'NSArray') {
            ...
            try {
                // try anyway as something has happend with classname not coming through will fail on json.parse if incorrect
                let test_ascii_string = NSString.alloc().initWithDataEncoding(data, NSASCIIStringEncoding).toString();
                
                if (JSON.parse(test_ascii_string)) {
                    data = test_ascii_string
                }
